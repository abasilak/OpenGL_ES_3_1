...
final class TGAReader
{
    ...
    static final Order ABGR = new Order(0, 8, 16, 24);

    static int getWidth(byte[] buffer) {
        return (buffer[12] & 0xFF) | (buffer[13] & 0xFF) << 8;
    }

    static int getHeight(byte[] buffer) {
        return (buffer[14] & 0xFF) | (buffer[15] & 0xFF) << 8;
    }

    static int [] read(byte[] buffer, Order order) throws IOException {
